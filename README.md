...
The Helm package for LinShare.


## Notice about LinShare

It is possible to customize the behavior of the UI component.  
...
Then, create a config map and reference it in the **values.yml** file.

```bash
kubectl create configmap \
	linshare-ui-user-js \
	--from-file=linshare.ui-user.config.js=path/to/linshare.ui-user.config.js
```

It is also possible to add extra resources, such as logos and images.

```bash
kubectl create configmap \
	linshare-ui-user-resources \
	--from-file=path/to/logo1.jpg \
	--from-file=path/to/background.jpg
```

It loads the resources under `/usr/local/apache2/htdocs/linshare-ui-user/custom-resources`.
You can then reference them in the JS configuration (to be loaded as a config map -
see above).
